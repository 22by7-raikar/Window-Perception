...
curr_dir = os.path.dirname('/home/ankush/Desktop/abhardwaj_p3a/')
...
#    node_tree.links.new(principled_node.outputs["BSDF"], material_output.inputs["Surface"])
...
def create_window(x, y, z, xdelta, ydelta, zdelta, qw, qx, qy, qz, xangdelta, yangdelta, zangdelta, text_path):
    ...
    window.rotation_quaternion = [1,0,0,0]
    ...
    if text_path:
        ...
        transparent_shader = window_material.node_tree.nodes.new('ShaderNodeBsdfTransparent')
    
        mix_shader = window_material.node_tree.nodes.new('ShaderNodeMixShader')
    
        # Link the nodes
        window_material.node_tree.links.new(shader_node.inputs["Base Color"], image_texture.outputs["Color"])
        window_material.node_tree.links.new(mix_shader.inputs[1], transparent_shader.outputs["BSDF"])
        window_material.node_tree.links.new(mix_shader.inputs[2], shader_node.outputs["BSDF"])
        window_material.node_tree.links.new(window_material.node_tree.nodes["Material Output"].inputs["Surface"], mix_shader.outputs["Shader"])
        
        # Use the alpha channel from the image texture as the mix factor
        window_material.node_tree.links.new(mix_shader.inputs[0], image_texture.outputs["Alpha"])
